...
solver_channel_activation_concentration_processor(struct TreespaceTraversal *ptstr, void *pvUserdata)
{
	...
	if (pmcd->iStatus == 1)
	{
		...
		if (ppistKinetic)
		{
			...
			dStep = (pcac->pgc.gc.htg.hi.dEnd - pcac->pgc.gc.htg.hi.dStart) / dEntries;
	...
	else if (pmcd->iStatus == 4)
	{
		...
		dStep = (pcac->pac.ca.htg.hi.dEnd - pcac->pac.ca.htg.hi.dStart) / dEntries;
...
solver_channel_concentration_processor(struct TreespaceTraversal *ptstr, void *pvUserdata)
{
	...
	if (pmcd->iStatus == 1)
	{
		...
		if (ppistKinetic)
		{
			...
			dStep = (pcc->pac.ca.htg.hi.dEnd - pcc->pac.ca.htg.hi.dStart) / dEntries;
