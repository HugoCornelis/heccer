...
int HeccerMechanismCompile(struct simobj_Heccer *pheccer)
{
	...
	for (iSchedule = 0 ; iSchedule < pheccer->inter.iCompartments ; iSchedule++)
	{
		...
		case MATH_TYPE_CallOut_conductance_current:
		{
		    ...
		    SETMAT_CALLOUT(iMathComponent, pcall);
...
/* 			    (pcsm->parameters.dTau1 != pcsm->parameters.dTau2), */
		    ...
		    // \todo so it looks as if here we should keep an
		    // index of where in memory iDiscreteTarget is
		    // located, and during connect, ie after DES has
		    // been constructed, fetch the correct
		    // iDiscreteTarget from DES.

		    // \todo need for a table that translates synchan
		    // number to iMops.

		    // \todo given the serial, addressing can
		    ...
		    SETMAT_SPRINGMASS(iMathComponent, pcsm->dInitX, pcsm->dInitY, dNextEvent, pheccer->dStep * pcsm->dFrequency);
			...
			SETMAT_FLUXPOOL(iMathComponent, 0.0);
			...
			SETMAT_STORESINGLECHANNELCURRENT(iMathComponent, 0.0, 0.0);
		...
		case MATH_TYPE_InternalNernst:
		{
		    ...
		    SETMAT_INTERNALNERNST(iMathComponent, pin->dInitPotential);
		...
		case MATH_TYPE_ChannelAct:
		{
			...
			SETMAT_POWEREDGATECONCEPT(iMathComponent, dInitActivation);
		    }
...
/* 				 "Compilation of MATH_TYPE_ChannelConc failed, illegal initial gate state"); */
		...
		case MATH_TYPE_ExponentialDecay:
		{
		    ...
		    SETMAT_EXPONENTIALDECAY(iMathComponent, pexdec->dInitValue);
...
/* 			iSource2Targets = EventDistributorSerial2Index(ped, ADDRESSING_HECCER_2_NEUROSPACES(iSerial)); */
		    ...
		    SETMAT_EVENTGENERATE(iMathComponent, dRefractoryTime, dSpike);
