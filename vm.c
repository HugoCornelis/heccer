	...
	if (phciCurrent)
	{
		...
		if (phct->iFormatted)
		{
			...
			else if (phciCurrent->iValue == HECCER_MOP_SPRINGMASS)
			{
				...
				&& pmops->pdEvents[0])
			    {
				sprintf(pc, " %i %g %i %i %i %g", pmops->iEvent, pmops->pdEvents[pmops->iEvent], pmops->iDiscreteSource, pmops->iDiscreteTarget, pmops->iTable, pmops->dFrequency);
			    }
			    else
			    {
				sprintf(pc, " %i (nil) %i %i %i %g", pmops->iEvent, pmops->iDiscreteSource, pmops->iDiscreteTarget, pmops->iTable, pmops->dFrequency);
			    }
			}
